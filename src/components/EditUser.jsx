...
const EditUser = () => {
      ...
      try {
        ...
        setFormData({
          user: {
            ...
            profile_pictures: response.data.profile_pictures || []
          },
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      formDataUpload.append('upload_preset', 'your_cloudinary_upload_preset'); // Replace with actual Cloudinary upload preset
      ...
      const updatedProfilePictures = [...formData.user.profile_pictures, imageUrl];
      await axios.put(
        `https://backend-nm1z.onrender.com/api/admin/auth/users/edit/${id}`,
        { profile_pictures: updatedProfilePictures },
        {
          headers: {
      ...
      setFormData(prev => ({
        ...prev,
        user: { ...prev.user, profile_pictures: updatedProfilePictures }
      }));
    ...
    <div className="min-h-screen bg-gradient-to-r from-blue-100 to-purple-100 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8 p-6 bg-white rounded-lg shadow-lg">
          <h2 className="text-2xl font-semibold text-indigo-600 mb-6">Profile Picture</h2>
          <div className="flex flex-col sm:flex-row gap-6">
            {formData.user.profile_pictures.length > 0 && (
              <div className="flex-shrink-0">
                <label className="block text-sm font-medium text-gray-700 mb-2">Current Picture</label>
                <img
                  src={formData.user.profile_pictures[formData.user.profile_pictures.length - 1]}
                  alt="Current Profile"
                  className="w-32 h-32 object-cover rounded-full border-2 border-indigo-200"
                />
              </div>
            )}
              ...
              {previewUrl && (
                <div className="mt-4">
                  ...
                  <img
                    src={previewUrl}
                    alt="Preview"
                    className="w-32 h-32 object-cover rounded-full border-2 border-indigo-200 mt-2"
                  />
                </div>
              )}
              {selectedFile && (
